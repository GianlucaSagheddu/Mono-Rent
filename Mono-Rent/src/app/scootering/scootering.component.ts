...
export class ScooteringComponent implements OnInit {
  ...
  partecipa(IdOfferta: number): boolean {
    const headers = new HttpHeaders({
      'Content-Type': 'application/x-www-form-urlencoded'
    ...
    const params = new HttpParams()
      .set('idRichiesta', IdOfferta.toString())
      .set('idUtente', this.cookieService.get('ID'));
    ...
    var parameter = JSON.stringify({
      idRichiesta: IdOfferta,
      idUtente: this.cookieService.get('ID')
    });

    this.http.post('https://3000-d670e502-c231-409e-b2f8-68e3b042a9da.ws-eu0.gitpod.io/partecipaS',null, options  )
      .subscribe(data => {
        this.data = data;
